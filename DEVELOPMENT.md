...
| 論点 | 決定 | 理由 |
|---|---|---|
| 次元・視点 | 2D サイドビュー(テラリア的) | 体裁を整えやすく、掘る・拾うが直感的 |
...
- **ボタンは少なく**: 移動 / ジャンプ / 掘る / 攻撃 / 決定 の5操作以内。キーボードとゲームパッドの両方に最初から対応する
...
- サイドビューの2Dマップを探索し、ブロックを掘って素材を集める
...
- ノイズ生成による1バイオームのマップ(土 / 石 / 木 / 草 の4ブロック)
- プレイヤー: 移動、ジャンプ、掘る、近接攻撃
...
| バイオーム・地下・天候 | `scripts/core/map_generator.gd` | パラメータ追加かレイヤー追加で対応 |
...
`TileMapLayer` を使う(`TileMap` は非推奨)。`MapGenerator` はノード非依存の純粋ロジックにして、seed を渡せば同じマップが出るようにする。
...
│   ├── player/
│   ├── enemies/
│   ├── world/       World.tscn(TileMapLayer を持つ)
│   └── ui/
├── scripts/
...
`git init` は未実施。M0 の最初に実行し、以降はマイルストーンごとに区切りのコミットを作る。
...
| M1 | 昼フェーズ: `MapGenerator` + `TileMapLayer`、プレイヤーの移動 / ジャンプ / 掘る、アイテム拾得、日没タイマー | 素材を集めて日没を迎えられる |
